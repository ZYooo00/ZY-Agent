...
const APP_VERSION = '26.06.07e'; // 格式：YY.MM.DD
...
const CHANGELOG = [
  {
    ...
    date: '2026-06-07',
    changes: [
      '【修正】備盤：效期「到期當天」仍可正常備盤，顯示橘色警告「今日到期（明起停用）」，隔天才鎖定',
      '【修正】備盤：開封逾期批號登記新開瓶後備盤，剩餘量不再被強制歸零（幽靈歸零 Bug）',
      '【修正】備盤：開封逾期批號現在固定顯示在最頂部（FEFO 排序），開新瓶後不再發生跳位',
      '【修正】備盤：開封逾期批號登記新開瓶時，舊殘液自動作廢，不再復活累加（80mL 合體 Bug）',
      '【修正】備盤：取消開瓶後，開封日精準還原至上一次狀態，不再卡死（開封日失憶 Bug）',
      '【修正】備盤：紅色按鈕路徑取消開瓶後，過期殘液正確吐回（Red Button Paradox）',
      '【修正】備盤：混批「第一批取用量」空白送出時，系統提示必填，不再幽靈少扣',
      '【修正】備盤：混批「第一批取用量」不得超過帳面剩餘量與今日需用量',
      '【修正】備盤：送出失敗回滾時，_manuallyOpened 一併還原，取消開瓶按鈕不再消失',
      '【修正】備盤：送出 Firebase 期間 UI 凍結，防止手滑點擊污染資料（Ghost Click）',
